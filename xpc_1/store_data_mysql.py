...
class StoreData(object):
	...
	def check_server_available(self, server_sequence):
		try:
			...
			sql = "select * from cmdb_server\
			 where cmdb_server_sequence = '%s' " % server_sequence
			cursor.execute(sql)
			rs = cursor.fetchall()
			if len(rs) != 0:
				return True
	...
	def insert_a_newserver(self, server_sequence, server_dict_data):
		try:
			...
			(cmdb_server_sequence, \
				...
				cmdb_server_diskinfo) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s')" % ( server_sequence, server_dict_data['server_arch_type'], server_dict_data['server_vm_type'], server_dict_data['server_machine_model'], server_dict_data['server_machine_type'], server_dict_data['server_cpuinfo'], server_dict_data['server_memoryinfo'], server_dict_data['server_diskinfo'])
			print sql_insert
	...
	def update_a_server(self, server_sequence, server_dict_data):
		try:
			...
			WHERE cmdb_server_sequence = '%s' " %(server_dict_data['server_arch_type'], server_dict_data['server_vm_type'], server_dict_data['server_machine_model'], server_dict_data['server_machine_type'], server_dict_data['server_cpuinfo'], server_dict_data['server_memoryinfo'], server_dict_data['server_diskinfo'], server_sequence)
			print sql_update
